...
- **工具**：技術指標、10 大策略投票、新聞情緒、多時間框架分析
- **特殊感知**：延伸行情偵測（>3×ATR 要求回調確認；>5×ATR 降低信心）、竭盡反轉模式
- **權限**：建議權（不能直接下單）
- **模型**：Claude Sonnet
...
- **Momentum Exit**：持倉 >2h 且無浮盈、RSI 回中 → 主動平倉
...
- **淘汰機制**：勝率 < 35% 自動退場
- **衝突懲罰**：研究員有反向假設時，分析師信心自動扣分（-0.05/個）
...
### 6. 延伸行情保護
當價格已單邊移動超過 3×ATR，分析師自動要求回調確認才能進場；超過 5×ATR 時強制降低信心。同時有三層追空保護 Guard：
```
Guard 1: 連續同方向 ≥5 筆 → 信心 ×0.7；≥8 筆 → 強制 hold
Guard 2: RSI 過度延伸（RSI<25）→ 倉位縮減至 30%
Guard 3: 18-24 UTC 高風險時段 → 倉位縮減至 50%
```
避免在極端行情中盲目追漲殺跌。
...
入場 止損 ~2%（Claude 根據波動率決定）
  ...
  │   └─ 低波動（ATR<0.5%）→ callback 收緊至 0.2×ATR
  │
  ├─ 有浮盈 + 止損距現價 <0.3% → 最小寬度保護（不觸發）
  │
  ├─ 有浮盈 → 時間止損不生效
  │
  ├─ 止盈達標 + 強趨勢 → TP 延伸（最多 3 次，動態步幅）
  │
  └─ 持倉 >2h + 無浮盈 + RSI 回中 → Momentum Exit（守護者主動平倉）
```

止損觸發時有 0.3% 滑點上限保護，實際出場不會遠超預設止損價。所有參數基於 ATR 動態調整，不是固定值。
...
```
每 10 分鐘：
  DataCollector → 10 策略投票 → Claude 分析 → Critic 把關 → 執行

每 3 分鐘：
  Position Manager → 評估持倉 → 讀布告欄 → 仲裁後決策（含 Momentum Exit）

每 10 秒：
  Price Monitor → SL/TP/Breakeven/Lock2/Trail/Shadow/TP延伸 追蹤
...
|---|---|---|
| 單筆止損 | ~2% | 平倉 + 冷卻 90min |
| 連續止損加強 | 同方向第 2 次 SL | 冷卻延長至 180min |
| 每日虧損 | 5% | 全天暫停 |
| 最大回撤 | 20% | 暫停至隔天 |
| 帳戶硬止損 | 25% | 永久停機 |
| Regime 禁止 | 極端恐慌禁做空 | 強制 hold |
| Conf Deadzone | 動態封鎖差劣區間 | Shadow + 自動解封 |
| Guard 1 連續追方向 | ≥5 筆 / ≥8 筆 | 信心 ×0.7 / 強制 hold |
| Guard 2 RSI 竭盡 | RSI<25 追空 | 倉位縮至 30% |
| Guard 3 高風險時段 | 18-24 UTC | 倉位縮至 50% |
...
- **通知**：Discord Webhook（4 頻道分流：交易 / 網格 / 警告 / 日報）
- **監控**：自建 Web Dashboard（Flask + 靜態 HTML，拆分 5 模組）
...
6. **追空保護系統**：-5.72% 的止損讓系統反思——方向對，但在 7×ATR 延伸後才進場。加入延伸行情感知、RSI 竭盡偵測、時段 Guard，強迫系統在極端行情中「等一下」
